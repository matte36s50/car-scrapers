...
def extract_proper_model(model_text, make_text=None):
    ...
    if model_str.upper() == 'AMG' or (len(model_str) < 3 and 'AMG' in model_text.upper()):
        # Try to extract more context - look for alphanumeric before AMG
        amg_match = re.search(r'([A-Z0-9]+(?:\s+[A-Z0-9]+)*)\s+AMG', model_text, re.IGNORECASE)
        if amg_match:
            return f"{amg_match.group(1)} AMG"
        # If still just AMG, use original as fallback
