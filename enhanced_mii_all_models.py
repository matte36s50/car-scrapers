...
def clean_sale_amount(sale_text: str, sale_type: Optional[str] = None) -> Optional[int]:
    ...
    Handles common CNB scraping issues:
    - Pattern 1: 7-digit numbers ending in 09-19 (e.g., $95,00011 → $95,000)
    - Pattern 2: High prices (>500k) with suspicious endings
    - Pattern 3: 8+ digit numbers (take first 6 digits)
    - Removes $ and commas
...
if __name__ == "__main__":
    success = main()
    exit(0 if success else 1), '').replace(',', '').strip()
    ...
    # PATTERN 1: 7 digits with suspicious ending (09-19)
    # This catches: $95,00011 → 9500011 → $95,000
    if digit_count == 7 and 9 <= last_two <= 19:
        amount = amount // 100
    
    # PATTERN 2: High prices (>500k) with suspicious patterns
    # This catches: $1,234,567 where last digits are 9,10,11,12,123,234,etc.
    elif amount > 500000 and last_three in [9, 10, 11, 12, 123, 234, 345, 456, 567, 678, 789]:
        amount = amount // 100
    
    # PATTERN 3: 8+ digits (likely multiple errors)
    # Take first 6 digits: $12345678 → $123456
    elif digit_count >= 8:
        amount = int(str(amount)[:6])
